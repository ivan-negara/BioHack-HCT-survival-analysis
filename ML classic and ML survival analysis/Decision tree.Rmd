...
```{r Tema 7, warning=FALSE, include=FALSE}
  #remove cols
...
predicted_train <- predict(fit, pre_predict_train, type = "class")
table_mat_train <- table(pre_predict_train$aGHVD_clear, predicted_train)
accuracy_Test_train <- sum(diag(table_mat_train)) / sum(table_mat_train)
cm_train <- confusionMatrix(table_mat_train, positive = "severe")
draw_confusion_matrix(cm_train)
```


After increasing depth of the decision tree to 10
```{r echo=FALSE}
